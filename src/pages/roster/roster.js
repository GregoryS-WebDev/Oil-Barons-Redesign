...
const categories = ["PITCHERS","CATCHERS","INFIELDERS","OUTFIELDERS","COACHES"];
...
function extendHandedness(handedness) {
    if (handedness.toUpperCase() === "L") {
      return "Left";
    } else if (handedness.toUpperCase() ===  "R") {
      return "Right";
    } else {
      return "NA";
    }
  ...
  const icons = {
      ...
      twitter: `
        <a href="#" target="_blank" rel="noopener">
            <img src="../../assets/images/icons/x-icon.png" alt="player's twitter" />
        </a>`,
      instagram: `
        <a href="#" target="_blank" rel="noopener">
            <img src="../../assets/images/icons/instagram-icon.png" alt="player's instagram" />
        </a>`,

      facebook: `
        <a href="#" target="_blank" rel="noopener">
            <img src="../../assets/images/icons/facebook-icon.png" alt="player's facebook" />
        </a>`
  };
...
function buildPlayerCard(player) {
    ...
    const playerInfo = el('p', 'info', 
        `B/T: ${player.bats}/${player.throws} &nbsp; Ht: ${player.height} &nbsp; Wt: ${player.weight} <br />
        DOB: ${formatBirthday(player.birthday, player.birthmonth, player.birthyear)} <br />
...
function buildCoachCard(coach) {
    ...
    const coachInfo = el('p', 'info', 
        `${coach.position} <br />
        DOB: ${formatBirthday(coach.birthday, coach.birthmonth, coach.birthyear)} <br />
        FROM: ${coach.hometown}`
    );
...
function render(filter="All"){
    ...
    categories.forEach(category=>{
        ...
        if(category.toUpperCase()==='COACHES'){
            globalCoaches.forEach(c=> group.appendChild(buildCoachCard(c)));
        } else {
        globalRoster
...
function openModal(obj, isCoach) {
  ...
  modalBio.textContent = obj.bio || ''; // hometown ??
  ...
  if (isCoach) {
    ...
    modalJerseyPos.innerHTML = `
      #${obj.jersey}&nbsp; <span id="separator">|</span> &nbsp;${obj.position}
    `;
    modalMeta.innerHTML = `
